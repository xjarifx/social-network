...
describe('Property 1: Fault Condition - Pro User Downgrade Flow', () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        fc.record({
          ...
          stripeCurrentPeriodEndAt: fc.option(fc.date().map(d => d.toISOString()), { nil: null }),
        ...
        async (proUserStatus: BillingStatus) => {
          ...
          vi.mocked(billingAPI.downgradeToFree).mockResolvedValue(downgradeResponse);
          ...
          const downgradeButtons = await screen.findAllByRole('button', { name: /downgrade/i });
          ...
          await waitFor(() => {
            expect(billingAPI.downgradeToFree).toHaveBeenCalled();
          }, { timeout: 3000 });

          // REQUIREMENT 2.3: Backend processes downgrade and returns updated status
          // (Verified by mock returning downgradeResponse)
          expect(vi.mocked(billingAPI.downgradeToFree)).toHaveBeenCalledTimes(1);
  ...
  it('Pro users see enabled Downgrade button on Free plan card', async () => {
    ...
    const downgradeButton = await screen.findByRole('button', { name: /downgrade/i });
    expect(downgradeButton).toBeInTheDocument();
    expect(downgradeButton).not.toBeDisabled();
  });
   ...
   * 
   * This test checks that the billingAPI has a downgradeToFree method,
   * which is currently missing from the API client.
   */
  it('billingAPI has downgradeToFree method', () => {
    // REQUIREMENT 1.3, 2.2: Frontend API method must exist
    expect(billingAPI).toHaveProperty('downgradeToFree');
    expect(typeof billingAPI.downgradeToFree).toBe('function');
  });
});
